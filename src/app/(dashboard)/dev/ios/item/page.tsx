...
export default function IosItemPageWrapper(){
  return <Suspense fallback={null}><IosItemPage/></Suspense>;
}

function IosItemPage(){
  const supabase = createClient();
  const router = useRouter();
  const params = useSearchParams();
  const editId = params.get('id') || undefined;
  const { iosVersion: selectedVer, iosVersions: allVersions, aosVersion, aosVersions, serverVersion, serverVersions, userName, userDept, userEmail } = useVersion();
  ...
  const aosVersionList = useMemo(()=>aosVersions.filter((v:any)=>!v.completed_at).map((v:any)=>v.version),[aosVersions]);
  const serverVersionList = useMemo(()=>serverVersions.filter((v:any)=>!v.completed_at).map((v:any)=>v.version),[serverVersions]);
  const crossVersionList: Record<string,string[]> = { AOS: aosVersionList, SERVER: serverVersionList };
  const crossDefaultVersion: Record<string,string> = { AOS: aosVersion, SERVER: serverVersion };
  ...
  const OTHER_PLATFORMS = ['AOS','SERVER'];
    ...
    if(editId){
      ...
      if(newItem?.id){
        ...
        for(const cp of crossWith){
          const cpVersion = crossVersions[cp] || (cp==='AOS' ? aosVersion : cp==='SERVER' ? serverVersion : p.version);
    ...
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950">
      ...
      <div className="sticky top-0 z-10 bg-white dark:bg-neutral-900 border-b border-neutral-200 dark:border-neutral-700 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button onClick={()=>router.push('/dev/ios')} className="p-1.5 rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-800">
      ...
      <div className="sticky bottom-0 bg-white dark:bg-neutral-900 border-t border-neutral-200 dark:border-neutral-700 px-6 py-4 flex justify-between max-w-6xl mx-auto">
        ...
        <div className="flex gap-2">
          <button onClick={()=>router.push('/dev/ios')} className="px-4 py-2 text-sm text-gray-600 hover:bg-gray-100 dark:hover:bg-neutral-800 rounded-lg">취소</button>
