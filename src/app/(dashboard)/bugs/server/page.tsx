...
import type { ServerBug, Developer, Priority, FixStatus } from '@/lib/types/database';
export default function ServerBugsPage() {
  const supabase = createClient();
  const [items, setItems] = useState<ServerBug[]>([]);
  ...
  const del=async()=>{if(!editId||!confirm('삭제?'))return;await supabase.from('server_bugs').delete().eq('id',editId);after();};
  return(<div>
    <div className="flex items-center justify-between mb-4">
      <div><h1 className="text-xl font-bold text-gray-900">서버 오류</h1><p className="text-xs text-gray-500 mt-1">AOS + iOS 양쪽 서버 오류</p></div>
      <button onClick={()=>{setEditId(null);setShowForm(true);}} className="flex items-center gap-1.5 bg-purple-600 text-white text-sm px-4 py-2 rounded-lg hover:bg-purple-700"><Plus size={16}/>오류 추가</button>
    </div>
    ...
    {(showForm||editId)&&<BugModal supabase={supabase} devs={devs} editId={editId} table="server_bugs" onClose={close} onSaved={after} onDel={del}/>}
  </div>);
}
