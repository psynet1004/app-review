...
export default function AppBugsPage() {
  ...
  const devTeam = useMemo(()=>developers.filter(d=>['개발팀','서버(백앤드)','서버(시스템)'].includes(d.department)),[developers]);
  ...
  const SectionHeader=({title,count,color,sectionKey,onAdd}:{title:string;count:number;color:string;sectionKey:string;onAdd:()=>void})=>(
    <div className={`flex items-center justify-between py-3 px-4 ${color} rounded-t-xl cursor-pointer select-none`} onClick={()=>toggle(sectionKey)}>
      <div className="flex items-center gap-2">{collapsed[sectionKey]?<ChevronDown size={16} className="text-white/70"/>:<ChevronUp size={16} className="text-white/70"/>}<h2 className="text-sm font-bold text-white">{title}</h2><span className="text-xs text-white/70 bg-white/20 px-2 py-0.5 rounded-full">{count}건</span></div>
  ...
  return(<div className="space-y-6">
    <div><h1 className="text-xl font-bold text-gray-900">앱 오류</h1><p className="text-xs text-gray-500 mt-0.5">AOS / iOS 앱 오류</p></div>
    <div className="rounded-xl border border-gray-200 overflow-hidden">
      ...
      {!collapsed.aos&&<DataTable data={aosBugs} columns={makeCols('AOS')} selectable selectedIds={selAos} onSelectionChange={setSelAos} searchKeys={['location','description']} searchPlaceholder="AOS 오류 검색..." emptyMessage={loading?'로딩 중...':'없음'}/>}
    </div>
    <div className="rounded-xl border border-gray-200 overflow-hidden">
      <SectionHeader title="🍎 iOS 앱 오류" count={iosBugs.length} color="bg-blue-600" sectionKey="ios" onAdd={()=>setShowForm({platform:'iOS'})}/>
      {!collapsed.ios&&<DataTable data={iosBugs} columns={makeCols('iOS')} selectable selectedIds={selIos} onSelectionChange={setSelIos} searchKeys={['location','description']} searchPlaceholder="iOS 오류 검색..." emptyMessage={loading?'로딩 중...':'없음'}/>}
    </div>
    {showForm&&<BugModal supabase={supabase} devTeam={devTeam} editId={showForm.id} platform={showForm.platform}
      defaultVersion={showForm.platform==='AOS'?aosVersion:iosVersion} versionList={getVersionList(showForm.platform)} userName={userName} userDept={userDept}
...
function BugModal({supabase,devTeam,editId,platform,defaultVersion,versionList,userName,userDept,onClose,onSaved,onDel}:any){
  const [f,sf]=useState({version:defaultVersion||'',location:'',description:'',priority:'보통' as Priority,department:userDept||'',reporter:userName||'',developer_id:'',fix_status:'미수정' as FixStatus,note:''});
  ...
  return(<div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={onClose}><div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
    ...
    <div className="p-6 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4"><DevSel l="개발담당" v={f.developer_id} c={v=>sf(p=>({...p,developer_id:v}))} devs={devTeam}/><Sel l="수정결과" v={f.fix_status} c={v=>sf(p=>({...p,fix_status:v as FixStatus}))} opts={['미수정','수정중','수정완료','보류'].map(s=>({v:s,l:s}))}/></div>
...
function VerSel({l,v,c,versions}:{l:string;v:string;c:(v:string)=>void;versions:string[]}){return(
  <div><label className="block text-xs font-medium text-gray-600 mb-1">{l}</label><select value={v} onChange={e=>c(e.target.value)} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500">
    {v&&!versions.includes(v)&&<option value={v}>{v}</option>}{versions.map(ver=><option key={ver} value={ver}>{ver}</option>)}</select></div>);}
function DevSel({l,v,c,devs}:{l:string;v:string;c:(v:string)=>void;devs:any[]}){
  ...
  return(<div><label className="block text-xs font-medium text-gray-600 mb-1">{l}</label>
    <select value={v} onChange={e=>c(e.target.value)} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500">
      ...
      {groups.map(g=>(<optgroup key={g.label} label={`── ${g.label} ──`}>{g.items.map(d=><option key={d.id} value={d.id}>{d.name} ({d.role})</option>)}</optgroup>))}
    </select></div>);}
