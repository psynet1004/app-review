...
export default function CommonBugsPage() {
  ...
  return(<div className="space-y-4">
    <div className="flex items-center justify-between">
      <div><h1 className="text-xl font-bold text-gray-900">공통 오류</h1><p className="text-xs text-gray-500 mt-0.5">AOS + iOS 양쪽 공통으로 발생하는 오류</p></div>
    ...
    <DataTable data={items} columns={cols} selectable selectedIds={selected} onSelectionChange={setSelected}
      searchKeys={['location','description','reporter']} searchPlaceholder="공통 오류 검색..." emptyMessage={loading?'로딩 중...':'등록된 오류 없음'}
    ...
    {showForm&&<BugModal supabase={supabase} devs={devs} editId={showForm.id} table="common_bugs" title="공통 오류" onClose={closeForm} onSaved={afterSave} onDel={handleDel}/>}
  </div>);
}

function BugModal({supabase,devs,editId,table,title,onClose,onSaved,onDel}:any){
