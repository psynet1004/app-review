...
export default function CommonBugsPage() {
  ...
  const { aosVersions, iosVersions, userName, userDept } = useVersion();
  ...
  return(<div className="space-y-4">
    <div className="flex items-center justify-between">
      <div><h1 className="text-xl font-bold text-gray-900">공통 오류</h1><p className="text-xs text-gray-500 mt-0.5">AOS + iOS 양쪽 공통 오류</p></div>
      <button onClick={()=>setShowForm({})} className="flex items-center gap-1.5 bg-orange-600 text-white text-sm px-4 py-2 rounded-lg hover:bg-orange-700"><Plus size={16}/>오류 추가</button>
    </div>
    <DataTable data={items} columns={cols} selectable selectedIds={selected} onSelectionChange={setSelected}
      searchKeys={['location','description','reporter']} searchPlaceholder="공통 오류 검색..." emptyMessage={loading?'로딩 중...':'없음'}
    ...
    {showForm&&<BugModal supabase={supabase} devTeam={devTeam} editId={showForm.id} table="common_bugs" title="공통 오류" versionList={allVers} userName={userName} userDept={userDept} onClose={closeForm} onSaved={afterSave} onDel={handleDel}/>}
  </div>);
}
function BugModal({supabase,devTeam,editId,table,title,versionList,userName,userDept,onClose,onSaved,onDel}:any){
  const [f,sf]=useState({version:versionList[0]||'',location:'',description:'',priority:'보통' as Priority,department:userDept||'',reporter:userName||'',developer_id:'',fix_status:'미수정' as FixStatus,note:''});
