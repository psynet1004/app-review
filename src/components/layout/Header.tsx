...
export const VersionContext = createContext<{
  ...
  userName: string; userDept: string;
}>({ aosVersion:'', iosVersion:'', setAosVersion:()=>{}, setIosVersion:()=>{}, aosVersions:[], iosVersions:[], refreshVersions:()=>{}, userName:'', userDept:'' });
...
function VersionDropdown({label,versions,selected,onSelect,color,refresh}:{
  ...
  const handleDelete=async(e:React.MouseEvent,id:string)=>{e.stopPropagation();if(!confirm('이 버전을 삭제할까요?'))return;await supabase.from('app_versions').delete().eq('id',id);refresh();};
  ...
  return (
    <div className="relative">
      <button onClick={()=>setOpen(!open)}
        ...
        <span className={`w-2 h-2 rounded-full ${cc.dot}`}/>{label} {selected||'미설정'}
      ...
      {open&&(<>
        ...
        <div className="absolute top-full left-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-xl z-30 min-w-[240px] py-1 overflow-hidden">
          ...
          <div className="max-h-60 overflow-y-auto">
            {versions.map(v=>(
              <div key={v.id} onClick={()=>{onSelect(v.version);setOpen(false);}}
                ...
                <div className="flex items-center gap-2.5">
                  {v.version===selected?<span className={`w-2 h-2 rounded-full ${cc.dot}`}/>:<span className="w-2 h-2 rounded-full bg-gray-200"/>}
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition">
                  <button onClick={(e)=>toggleComplete(e,v)} className="text-[10px] px-1.5 py-0.5 rounded bg-gray-100 hover:bg-gray-200 text-gray-500">{v.is_current?'해제':'완료'}</button>
